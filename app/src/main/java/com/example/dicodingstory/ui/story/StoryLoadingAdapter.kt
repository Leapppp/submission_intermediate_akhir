...
class StoryLoadingAdapter(private val retry: () -> Unit) : LoadStateAdapter<StoryLoadingAdapter.ViewHolder>() {
    ...
    class ViewHolder(itemView: View, retry: () -> Unit) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(loadState: LoadState) {
            ...
            if (loadState is LoadState.Error) {
                errorMsg.text = loadState.error.localizedMessage ?: itemView.context.getString(R.string.error_message_story)
            } else {
                errorMsg.text = ""
            }
        }
